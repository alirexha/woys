...
def main() -> int:
    ...
    _row("keepalive(ort) calls", "keepalive_calls", "8.0f")
    _row("keepalive(ort) p99 ms", "keepalive_p99_ms")
    _row("keepalive(torch) calls", "torch_keepalive_calls", "8.0f")
    _row("keepalive(torch) p50 ms", "torch_keepalive_p50_ms", "8.3f")
    _row("keepalive(torch) p99 ms", "torch_keepalive_p99_ms", "8.3f")
    _row("clock_lock floor (MHz)", "gpu_clock_lock_floor_mhz", "8.0f")
    _row("clock_lock ceiling (MHz)", "gpu_clock_lock_ceiling_mhz", "8.0f")
    print()
    print("---- shapes ----")
