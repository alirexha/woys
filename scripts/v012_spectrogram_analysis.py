...
def _detect_clicks(audio: np.ndarray, sr: int, *, window_ms: float = 4.0) -> np.ndarray:
    ...
    """
    win = max(8, int(round(sr * window_ms / 1000.0)))
    # Squared envelope.
...
def _click_rate_hypothesis_table(
    ...
    candidates = [
        ...
        ("2× chunk_seconds (subharmonic)", 2 * chunk_seconds * 1000.0),
        ("0.5× chunk_seconds (harmonic)", 0.5 * chunk_seconds * 1000.0),
    ]
    return sorted(
...
def _click_spectrum(audio: np.ndarray, sr: int, click_indices: np.ndarray) -> np.ndarray:
    ...
    if len(click_indices) > 0:
        ...
        click_avg = np.mean(np.array(click_specs), axis=0) if click_specs else np.zeros(n_fft // 2 + 1)
    else:
    ...
    if len(click_indices) >= 2:
        for prev, curr in zip(click_indices[:-1], click_indices[1:], strict=False):
            mid = (prev + curr) // 2
...
def _save_plots(
    ...
    ax.set_title(f"spectrogram (cyan = chunk_seconds={chunk_seconds}s boundaries; green = detected clicks)")
    ...
    if len(click_idx) >= 2:
        ...
        for label, pred_ms in [
            ...
            ("0.5× chunk", chunk_seconds * 500.0),
            ("2× chunk", chunk_seconds * 2000.0),
        ]:
    ...
    if len(click_idx) >= 2:
        ...
        ax.set_title("click vs steady-state spectrum (positive = click adds energy at this frequency)")
        ax.set_xscale("log")
        ax.set_xlim(20, sr / 2)
...
def main() -> int:
    ...
    print()
    print(f"==== v0.12.0 Phase 1 — sustained-vowel spectrogram analysis ====")
    ...
    if peak < 0.001:
        print(f"  [error] recording is essentially silent (peak < 0.001). "
              f"Did the engine produce output? Check WoysSink wiring.")
        return 2
    ...
    if len(click_idx) >= 2:
        print(f"  interval p50 {stats['median_interval_ms']:.2f} ms  ({1000.0 / stats['median_interval_ms']:.2f} Hz)")
        print(f"  interval p_mean {stats['mean_interval_ms']:.2f} ms")
        print(f"  interval std  {stats['std_interval_ms']:.2f} ms  CV={stats['cv']:.3f}")
        print(f"  interval min/max {stats['min_interval_ms']:.2f} / {stats['max_interval_ms']:.2f} ms")

    if len(click_idx) >= 2:
        print()
        print(f"  ---- mechanism candidates (predicted period vs observed median) ----")
        ...
        for label, predicted_ms, error_ms in ranked[:5]:
            ...
            print(f"  {label:40s} predicted={predicted_ms:7.2f} ms  error={error_ms:6.2f} ms  {marker}")
    ...
    if len(click_idx) < 5:
        ...
        print(f"  for this recording's noise floor. Consider repeating with a louder vowel.")
    else:
        ...
        if abs(median_ms - chunk_ms) < 5.0:
            ...
            print(f"    interval median {median_ms:.1f} ms within 5 ms of chunk_seconds {chunk_ms:.0f} ms")
            print(f"  → NSF reset at chunk boundaries IS the mechanism, OR another chunk-aligned")
            print(f"    process (writer thread cadence, SOLA crossfade boundary). Check the")
            print(f"    spectrum plot: f0-aligned excess = NSF phase reset; broadband excess =")
            print(f"    amplitude-discontinuity (writer/ring underrun pattern).")
        elif abs(median_ms - chunk_ms / 2) < 5.0:
            print(f"  matched mechanism: chunk_seconds/2 — half-chunk subharmonic, possibly two")
            print(f"    distinct artifacts at chunk_seconds spaced offset")
        elif abs(median_ms - chunk_ms * 2) < 5.0:
            print(f"  matched mechanism: 2× chunk_seconds — every-other-chunk pattern, possibly")
            print(f"    chunks alternate between 'good' and 'bad' alignment")
        elif abs(median_ms - 1024.0 / 48000.0 * 1000.0) < 1.5:
            print(f"  matched mechanism: PipeWire quantum boundary — ring-side issue, not engine")
        else:
            print(f"  no clean match to any predicted mechanism period")
